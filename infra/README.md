...
`.env.example` defaults are deliberately for services launched from the host via Gradle, so they use `localhost`. The matching `*_INTERNAL_*` values are for a future backend container on the Compose network. Authentication configuration is described in [the authentication convention](../docs/architecture/authentication.md): database and NexusFlow JWT secrets are injected by the runtime, while Google audience and Android client IDs are non-secret configuration.
