...
- 路由解析和结构校验传输输入，解析 `ActorContext`（若端点需要身份），调用一个应用层 use case，并将预期业务失败映射为 `KResponse`。路由不得直接持久化业务状态或调用第三方客户端。
...
| 条件 | HTTP 状态 | 说明 |
| --- | --- | --- |
| token 缺失或无效 | 401 | 不泄露租户或资源事实。 |
| 有效身份缺少权限或资源所有权 | 403 或不可区分的 404 | 选择并记录策略；列表接口按范围过滤。 |
| DTO / 输入结构无效 | 422 | 只返回安全的结构性信息。 |
| 幂等键与请求指纹不匹配 | 409 | 不创建第二次副作用。 |
| 并发或状态冲突 | 409 | 只在安全时返回当前版本/状态。 |
| 依赖不可用 | 503 | 不得伪装为业务成功或终态失败。 |

所有 JSON 响应使用 `KResponse(code, message, data)`，其中 body 的 `code` 必须与 HTTP 状态值一致。成功只填 `data`；失败只填安全的 `message`。HTTP 平台生成或校验 `X-Request-Id`，并将其回写响应。`StatusPages` 负责畸形请求与未处理异常，feature route 只映射其可预期的业务失败。
