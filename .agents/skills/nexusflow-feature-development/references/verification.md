...
| AI | use actual `:ai` tasks only once the Gradle module/runtime exists | deterministic guardrail tests, schema/contract tests, eval cases, provider-adapter tests after real source exists |
...
There is currently no implemented AI runtime and no `:ai` Gradle module in the baseline described by the repository docs. Do not invent an AI command as proof.
...
- no text implies an implemented AI runtime when the repository only has a reserved `ai/` root.
